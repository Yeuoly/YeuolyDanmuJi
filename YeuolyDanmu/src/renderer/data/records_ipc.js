 ...
 */
export function setListenner(channel,messager){ actions.extrs.setListenner(channel, messager); }

/**
 * @param {string} channel 指定的监听频道
 * @param {function(msg)} messager 监听器，将会移除该监听器
 */
export function removeListenner(channel,messager){ actions.extrs.removeListenner(channel,messager); }